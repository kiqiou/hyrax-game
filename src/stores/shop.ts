...
export const useShopStore = defineStore('shop', () => {
  ...
  const getItem = (id: ShopItem['id']) => items.value.find(item => item.id === id)
  ...
  const availableItems = computed(() => items.value.filter(item => !item.bought))
  const purchasedItems = computed(() => items.value.filter(item => item.bought))
